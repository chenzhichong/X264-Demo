...
int CCaptureImage::Initialize( HWND hwnd, int Height, int Width, IMAGE_CALLBACK func, void *usedata )
{
	...
	if(FAILED(hr))
	{
		LOG_ERROR("创建" << CAPTURE_DEVICE_NAME[WHICH_DEVICE] << "失败！");
		return hr;
	}
	hr = m_pGraphBuilder->AddFilter(m_pGrabberDevFilter, CAPTURE_DEVICE_NAME[WHICH_DEVICE]);
	if(FAILED(hr))
	{
		LOG_ERROR("添加" << CAPTURE_DEVICE_NAME[WHICH_DEVICE] << "失败！");
		return hr;
	}
	...
	if(FAILED(hr))
	{
		LOG_ERROR("创建" << CROSSBAR_DEVICE_NAME[WHICH_DEVICE] << "失败！");
		return hr;
	}
	hr = m_pGraphBuilder->AddFilter(m_pCrossbarFilter, CROSSBAR_DEVICE_NAME[WHICH_DEVICE]);
	if(FAILED(hr))
	{
		LOG_ERROR("添加" << CROSSBAR_DEVICE_NAME[WHICH_DEVICE] << "失败！");
		return hr;
	}
	...
	if(FAILED(hr))
	{
		LOG_ERROR("创建Null Renderer Filter失败！");
		return hr;
	}
	hr = m_pGraphBuilder->AddFilter(m_pRenderFilter, L"Null Renderer");
	if(FAILED(hr))
	{
		LOG_ERROR("添加Null Renderer Filter失败！");
		return hr;
	}
	...
	if(FAILED(hr))
	{
		...
		LOG_ERROR("GetPin[" << CROSSBAR_DEVICE_NAME[WHICH_DEVICE] << ":" << CROSSBAR_DEVICE_PIN_OUT_NAME[WHICH_DEVICE] << "]失败！");
		return hr;
	}
	hr = GetPin(m_pGrabberDevFilter, CAPTURE_DEVICE_VEDIO_PIN_IN_NAME[WHICH_DEVICE], &pPinIn);
	if(FAILED(hr))
	{
		LOG_ERROR("GetPin[" << CAPTURE_DEVICE_NAME[WHICH_DEVICE] << ":" << CAPTURE_DEVICE_VEDIO_PIN_IN_NAME[WHICH_DEVICE] << "]失败！");
		return hr;
	}
	...
	if(FAILED(hr))
	{
		LOG_ERROR("GetPin[SampleGrabber:Input]失败！");
		return hr;
	}
	hr = m_pGraphBuilder->ConnectDirect(pPinOut, pPinIn, NULL);
	if(FAILED(hr))
	...
	if(FAILED(hr))
	{
		LOG_ERROR("GetPin[SampleGrabber:Output]失败！");
		return hr;
	}
	hr = GetPin(m_pRenderFilter, L"In", &pPinIn);
	if(FAILED(hr))
	{
		LOG_ERROR("GetPin[Null Render:In]失败！");
		return hr;
	}
	hr = m_pGraphBuilder->ConnectDirect(pPinOut, pPinIn, NULL);
	if(FAILED(hr))
...
HRESULT CCaptureImage::SetGrabberProperty()
{
	...
	if (FAILED(hr))
	{
		LOG_ERROR("显示GrabberDevFilter属性对话框失败！");
		return hr;
	}
	return hr;
}
